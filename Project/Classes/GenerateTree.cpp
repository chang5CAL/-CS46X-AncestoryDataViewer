...
std::vector<GenerateTree::Person> GenerateTree::parse(std::string fileLocation) {
	std::vector<Person> p;
...
void GenerateTree::placeNodes(Person p, std::vector<Person> family, Person parent) {
	...
	personNode->FindComponentByClass<TextRender>()->changeTextRender(name);
...
void GenerateTree::placeSpouse(Person p, std::vector<Person> family) {
	float offset = 0;
	for (int i = 0; i < p.treeLevel; ++i) {
		offset += levelMaxVal[i][0]; //The 0th node in each levelMaxVal is the max!
									 //Keep adding until you hit the tree level, so you move over enough.
	...
	if (!p.spouse.empty()) {
		...
		spouseNode->FindComponentByClass<UTextRender>()->changeTextRender(sName);
	...
	for (int i = 0; i < p.children.size(); ++i) {
		placeSpouse(family[p.children[i]],family);
	}
}
=======
// Fill out your copyright notice in the Description page of Project Settings.
...
>>>>>>> a9cef076874adf49240f8634173c751fad03ae8a
