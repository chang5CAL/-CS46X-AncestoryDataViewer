...
void ANodeSpawner::CreateNode(FVector location)
{
	...
	newNode->FindComponentByClass<UTextRenderComponent>()->SetText("Block2-CreateNode");
	...
	GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::White, TEXT("NodeSpawner After CreateNode"));
}

//Parser Functionality
int ANodeSpawner::search_by_id(std::vector<Person> p, FString str) {
	int i = 0;
	while (i < p.size()) {
...
int ANodeSpawner::findmaxLevel(std::vector<Person> p, FString sid) {
	double maxLev = 0;
	double curLev = 0;
...
void ANodeSpawner::setLevel(FString sid, int maxLevel) {
...
int ANodeSpawner::setSpace(FString sid) {
	int id = search_by_id(parsedData, sid);
	int curVal = 0;
	int maxVal = 0;
...
void ANodeSpawner::setPosition(FString sid, float position) {
	int id = search_by_id(parsedData, sid);
...
void ANodeSpawner::placeNodes(FString sid, int middle) {
	int id = search_by_id(parsedData, sid);
	float x = middle;
...
std::vector<FString> ANodeSpawner::findRoots(std::vector<Person> family) {
...
void ANodeSpawner::find_common_ancestor(FString p1, FString p2) {
